    ...
    <div class="space-y-6">
        ...
        <div class="bg-gradient-to-r from-blue-900 via-blue-800 to-blue-900 rounded-2xl p-6 md:p-8 text-white shadow-xl">
            <div class="flex flex-col md:flex-row items-start md:items-center justify-between">
                <div>
                    <h1 class="text-2xl md:text-3xl font-bold mb-2">Add Subtopic</h1>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
            <div class="bg-white rounded-xl p-6 shadow-lg">
                ...
                <h3 class="text-gray-500 text-sm font-medium mb-1">Total Subtopics</h3>
        ...
        <div id="rechargeForm" class="bg-white rounded-xl shadow-lg p-6" style="display: none;">
            ...
            <form class="space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <!-- Circle -->
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">Subtopic Title</label>
                        <input type="text" name="subtopicTitle" id="subtopicTitle"
                            class="w-full px-4 py-3 border-2 border-gray-200 rounded-lg focus:border-blue-500 focus:outline-none">
                    </div>

                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">Subtopic Slug</label>
        ...
        <div class="bg-white rounded-xl shadow-lg p-6">
            ...
            <!-- Sub Topic History Table -->
            <div class="overflow-x-auto">
                <table id="subTopicTable" class="display responsive nowrap" style="width:100%">
                    <thead class="bg-gradient-to-r from-blue-900 to-blue-800 text-white">
                        <tr>
                            <th class="px-6 py-4">#</th>
                            <th class="px-6 py-4">ParentId</th>
                            <th class="px-6 py-4">TopicId</th>
...
@push('scripts')
    ...
    <script>
        ...
        function applyRechargeFilters() {
            ...
            subTopicTable.clear().draw();

            const filteredData = subTopicData.filter(subTopic => {
                const subTopicDate = new Date(subTopic.date);
                const from = fromDate ? new Date(fromDate) : null;
                const to = toDate ? new Date(toDate) : null;

                return (!from || subTopicDate >= from) &&
                    (!to || subTopicDate <= to) &&
                    (!operator || subTopic.operator.toLowerCase() === operator.toLowerCase()) &&
                    (!status || subTopic.status === status);
            });

            subTopicTable.rows.add(filteredData).draw();
        }

        function resetSubTopicFilters() {
            $('#filterFromDate').val('');
            $('#filterToDate').val('');
            $('#filterOperator').val('');
            $('#filterStatus').val('');
            subTopicTable.clear().rows.add(subTopicData).draw();
        }

        function viewSubTopic(subTopic) {
            const modalContent = `
        <div class="space-y-4">
            <div>
                <h4 class="text-lg font-bold text-gray-900 mb-2">Sub Topic ID: <span class="font-mono font-semibold">${subTopic.id}</span></h4>
                <p class="text-gray-700">Date & Time: <span class="font-semibold">${subTopic.date}</span></p>
            </div>
            <div>
                <p class="text-gray-700">Mobile Number: <span class="font-mono font-semibold">${subTopic.mobile}</span></p>
                <p class="text-gray-700">Operator: <span class="font-semibold">${subTopic.operator}</span></p>
                <p class="text-gray-700">Plan Details: <span class="font-semibold  ">${subTopic.plan}</span></p>
                <p class="text-gray-700">Amount: <span class="font-bold text-gray-900">₹${subTopic.amount.toLocaleString()}</span></p>
